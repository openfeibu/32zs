...
class Sys extends Base
{
	...
	public function route_runadd()
	{
        ...
        $this->success('路由规则添加成功',url('admin/Sys/urlsetsys',array('page' => $p)),1);
	...
	public function route_runedit()
	{
        ...
        if($rst!==false){
            if(config('url_route_mode')=='2') Cache::rm('routes');
            $this->success('路由规则修改成功',url('admin/Sys/urlsetsys',array('page' => $p)));
        }else{
            $this->error('路由规则修改失败',url('admin/Sys/urlsetsys',array('page' => $p)));
        }
	}
	/*
     * 路由规则修改返回值操作
	...
	public function route_del()
	{
        ...
        if($rst!==false){
			$p=input('p',1,'intval');
            if(config('url_route_mode')=='2') Cache::rm('routes');
            $this->success('路由规则删除成功',url('admin/Sys/urlsetsys',array('page' => $p)));
        }else{
