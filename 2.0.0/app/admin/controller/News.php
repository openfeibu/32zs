...
class News extends Base
{
	...
	public function news_list()
	{
		...
		if(!empty($key)){
			if($keytype=='news_title'){
				...
				$map['admin_username']= array('like',"%".$key."%");
			}else{
				$map[$keytype]= $key;
			}
		}
	...
	public function news_del()
	{
		...
		if($rst!==false){
			...
			$this -> error("删除文章失败！",url('admin/News/news_list',array('page' => $p)));
		}
	}
    /**
     * 删除至回收站(全选)
	...
	public function news_alldel()
	{
		...
		$ids = input('n_id/a');
		if(empty($ids)){
			$this -> error("请选择删除文章",url('admin/News/news_list',array('page' => $p)));//判断是否选择了文章ID
		}
		...
		if($rst!==false){
			$this->success("成功把文章移至回收站！",url('admin/News/news_list',array('page' => $p)));
		}else{
			$this -> error("删除文章失败！",url('admin/News/news_list',array('page' => $p)));
		}
	}
	...
	public function news_back_alldel()
	{
		...
		$ids = input('n_id/a');
		if(empty($ids)){
			$this -> error("请选择删除文章",url('admin/News/news_back',array('page' => $p)));//判断是否选择了文章ID
		}
		if(is_array($ids)){//判断获取文章ID的形式是否数组
			$where = 'n_id in('.implode(',',$ids).')';
		}else{
		...
		if($rst!==false){
			$this->success("成功把文章删除，不可还原！",url('admin/News/news_back',array('page' => $p)));
		}else{
			$this -> error("文章彻底删除失败！",url('admin/News/news_back',array('p' => $p)));
		}
	}
