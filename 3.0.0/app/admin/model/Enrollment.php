...
class Enrollment extends Model
{
    ...
    public static function get_enrollment_major_list($school_id,$major_id)
    {
        return Db::name('enrollment')->alias('e')
            ->join(config('database.prefix').'recruit_major rm','rm.recruit_major_id = e.recruit_major_id')
            ->where(['e.school_id' => $school_id])
            ->where(function ($query) use ($major_id) {
                $query->where(array('e.major_ids' => array('LIKE' , '%,'.$major_id.',%')))
                    ->whereOr('e.major_ids',$major_id);
            })
            ->select();
    }
    public static function get_year_enrollment_major_list($school_id,$major_id)
    {
        return Db::name('enrollment')->alias('e')
            ->join(config('database.prefix').'recruit_major rm','rm.recruit_major_id = e.recruit_major_id')
            ->where(['e.school_id' => $school_id])
    ...
    public static function get_enrollment_major($school_id,$major_id)
    {
        return Db::name('enrollment')->alias('e')
            ->join(config('database.prefix').'recruit_major rm','rm.recruit_major_id = e.recruit_major_id')
            ->where(['e.school_id' => $school_id])
