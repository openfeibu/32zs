...
class Export extends Model
{
    ...
    public function university_score_list_export_pdf($field_titles=array(),$fields=array(),$data=array(),$table='Newfile',$title)
    {
        set_time_limit(120);
        if(empty($field_titles) || empty($data)) $this->error("导出的数据为空！");
        require_once(EXTEND_PATH . 'tcpdf/examples/lang/eng.php');
        require_once(EXTEND_PATH . 'tcpdf/RecruitScoreListTCPDF.php');
        $pdf = new \RecruitScoreListTCPDF('L', PDF_UNIT, 'A4', true, 'UTF-8', false);//新建pdf文件
        //设置文件信息
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor(config('pdf_common.author'));
        $pdf->SetTitle($table);
