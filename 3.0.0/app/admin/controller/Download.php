...
class Download extends Base
{
    public function member_excel()
    {
        ...
        $file_sub_path = ROOT_PATH.DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'excel'.DIRECTORY_SEPARATOR.'sample'.DIRECTORY_SEPARATOR;
    ...
    public function enrollment_excel()
    {
        header("Content-type: text/html;charset=utf-8");
        $file_name = '招生计划导入示例表格.xlsx';
        $file_sub_path = ROOT_PATH.DIRECTORY_SEPARATOR.'public'.DIRECTORY_SEPARATOR.'excel'.DIRECTORY_SEPARATOR.'sample'.DIRECTORY_SEPARATOR;
        $file_name = iconv("utf-8","gbk//IGNORE",$file_name);
        $file_path = $file_sub_path.$file_name;
        if (!file_exists($file_path)) {  //判断文件是否存在
