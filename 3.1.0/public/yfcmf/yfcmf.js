...
(function ($) {
    ...
    // 成绩管理
    $('body').on('change','.school_id_input',function () {
        load = layer.load(2);
        var $form = $(this).parents("form");
        var url = $form.attr('action')
        //2019.11.7
        var form = 'school_id='+$(this).val()+'&subject_id='+'&major_score_status='+'&search_key='
        console.log($form)
        ...
        $.ajax({
            ...
            success: function(data,status){
                ...
                if (!query) {
                    ...
                    history.pushState(null, "",
                        location.href.split("?")[0] + "?" + form);
                }
                window.location.href="http://new32.gdngs.cn/admin/university_score/score_list"+ "?" + form;
                if(typeof load!="undefined"){layer.close(load);}
                // $("#ajax-data").html(data);
    ...
    $('body').on('change','.fail_school_id_input',function () {
        load = layer.load(2);
        var $form = $(this).parents("form");
        //2019.11.7
        var form = 'school_id='+$(this).val()+'&subject_id='+'&major_score_status='+'&search_key='
        ...
        $.ajax({
            ...
            success: function(data,status){
                ...
                window.location.href="http://new32.gdngs.cn/admin/major_resit_score/fail_score_list"+ "?" + form;
                if(typeof load!="undefined"){layer.close(load);}
                // $("#ajax-data").html(data);
    ...
    $('body').on('change','input[name=major_score_status]',function () {

        load = layer.load(2);
        var $form = $(this).parents("form");
        $("input[name=major_score_status]").each(function(){
    ...
    $('body').on('change','.subject_id_input',function () {
        load = layer.load(2);
        var $form = $(this).parents("form");
...
$(function(){
	...
	$('.hanle_count').click(function(){
		var count = $("input[name='subject_name[]']").length;
		console.log(count);
		if($(this).hasClass('fa-plus-circle'))
		{
			...
			// {
				var k = count+1;
				var html = '<div class="form-group major_subject_group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 科目'+k+'： </label><div class="col-sm-10"><input type="text" name="subject_name[]" value="" /> <input type="text" name="max_score[]" value="" style="width:100px"/></div></div>';
		...
		if($(this).hasClass('fa-minus-circle'))
		{
			if(count >=4)
			{
				$(".major_subject_group").last().remove();
			}
		}
    ...
    $(".delete-subject").click(function () {
        var subject_id = $(this).attr('attr-id');
        layer.confirm("即将删除该科目和已录入的成绩，确定操纵吗？", {icon: 3}, function (index) {
            load = layer.load(1);
            $.ajax({
                type:"POST",
                url: "/admin/school/subject_delete",
	...
	$("body").on('blur','.major_score',function(){
        ...
        var subject_id = $this.parent().attr('subject-id');
        var html = '<img src="/public/img/loading.gif" id="loading_img">';
        $this.after(html);
        $.ajax({
            type:"POST",
            url: "/admin/score/score_runadd",
            data:{'member_list_id':member_list_id,'subject_id':subject_id,'score':value},
            success: function(data){
                $('#loading_img').remove();
                $this.after('<span class="save_span">已保存</span>');
    ...
    $('body').on('blur','.recruit_major_score',function(){
        member_list_id = $(this).attr('data-id');
        recruit_major_score = $.trim($(this).val());
        $this = $(this);
        var html = '<img src="/public/img/loading.gif" id="loading_img">';
        if(recruit_major_score.length > 0){
            var reg = /^[0-9]+.?[0-9]*$/;
            if(!reg.test(recruit_major_score)){
                ...
                $.ajax({
                    ...
                    data:{'member_list_id':member_list_id,'recruit_major_score':recruit_major_score},
                    success: function(data){
                        $('#loading_img').remove();
                        $this.after('<span class="save_span">已保存</span>');
                        $('.save_span').fadeOut('normal',function(){
    ...
    $("body").on('blur','.major_resit_score',function(){
        ...
        $.ajax({
            type:"POST",
            url: "/admin/MajorResitScore/fail_score_runadd",
            data:{'member_list_id':member_list_id,'subject_id':subject_id,'score':value},
            success: function(data){
                $('#loading_img').remove();
                $this.after('<span class="save_span">已保存</span>');
