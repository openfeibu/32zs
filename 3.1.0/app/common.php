...
function get_news($tag, $ispage=false, $pagesize=10, $type=null, $v=null, $where=array(), $currentPage=null)
{
    ...
    if (!empty($currentPage)) {
        $config['page']=intval($currentPage);
    }
    switch ($type) {
        case 'keyword':
...
/* 中职专业成绩html */
function major_score_desc($major_subject_name_arr, $major_score_arr)
{
    $html = '';
    foreach ($major_subject_name_arr as $key => $major_subject_name) {
        $v = isset($major_score_arr[$key]) ? $major_score_arr[$key] : '';
        $html .= "<span style='margin-right:5px'>" . $major_subject_name . ':' . $v . "</span>";
    }
    return $html;
}
/* 中职专业成绩转化数组 */
function handle_major_score_arr($major_subject_name_arr, $major_score_arr)
{
    $arr = array();
    foreach ($major_subject_name_arr as $key => $major_subject_name) {
        $arr[$major_subject_name] = isset($major_score_arr[$key]) ? $major_score_arr[$key] : '';
    }
    return $arr;
}
/* 根据身份证获取生日 */
...
//    $year = '2018';
    if($year)
    {
        ...
        $this_year = Cache::get('year') ? Cache::get('year') : date('Y');
    }
    if(isset($this_year) && !empty($this_year))
    {
        if($table_name)
        {
            return $table_name.".year = '".$this_year."'";
        }else{
...
function get_year_group(){
    $res = Db::name('enrollment')->order('year desc')->group('year')->limit(3)->column('year');
    return $res;
}
function get_year()
{
