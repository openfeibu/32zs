...
class MajorResitScore extends Model
{
    ...
    {
        $member_list = Db::name('recruit_major_score')->alias("ms")
        ...
        if($subject_list && count($subject_list)==1){
            // dump($subject_list);
            $member_list = $member_list->where('subject_id',$subject_list[0]['subject_id']);
        }
        $member_list = $member_list->where(get_year_where());
        $member_list_id_arr = $member_list->order('ms.recruit_major_score_status desc')->group('m.member_list_id')->column('m.member_list_id');
    ...
    {
        // echo $where;
        $score_list = Db::name('recruit_major_score')->alias("ms")
            ...
            ->order('m.major_id asc')
            ->order('m.member_list_id desc')
            ->group('m.member_list_id') // 2019.11.1
            ->field('m.member_list_nickname , m.member_list_username, m.member_list_id,m.year,mj.major_name,mj.major_name,m.major_id,m.school_id,mi.ZexamineeNumber');
            
        // $score_list->column('m.member_list_id');

        $score_list = $score_list->paginate(config('paginate.list_rows'),false,['query'=>get_query()]);
        $page = $score_list->render();
        $score_list = $score_list->all();

        $major_score_status1 = $major_score_status;
        foreach($score_list as $key => $member)
        {
            $major_score_arr = $major_subject_score_arr = array();
            ...
            $major_score_data = $this->get_member_subject_score($subject_list,$member['member_list_id'],$major_score_status1);
            // dump($major_score_data);
            $major_score_arr = $major_score_data['major_score_arr'];
            $major_score_status = $major_score_data['major_score_status'];
            $major_subject_score_arr = $major_score_data['major_subject_score_arr'];
    ...
    {
        // echo $major_score_status;
        $subject_id_arr = array_column($subject_list,'subject_id');
        $subject_score_list = DB::name('recruit_major_score')->where('member_list_id',$member_list_id)->where(array('subject_id' => array('in',$subject_id_arr)));
        if($major_score_status != NULL)
        {
            $subject_score_list->where('recruit_major_score_status',$major_score_status);
        }
        $subject_score_list = $subject_score_list->field('recruit_major_score,recruit_major_score_status,subject_id')->order('subject_id','asc')->select();
        ...
        foreach ($subject_list as $subject_key => $subject)
        {
            ...
            {
                $major_score = $subject_score_list[$subject['subject_id']]['recruit_major_score'];
                $major_score_status = $subject_score_list[$subject['subject_id']]['recruit_major_score_status'];
