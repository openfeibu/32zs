...
class Score extends Model
{
    ...
    // 2019 $one_subject_list  添加单一科目条件   major_score_status 1 已经审核 0 未 
	public function getMajorScoreList($map,$where,$search_key='',$subject_list=array(),$is_page = 1,$one_subject_list="",$major_score_status=null)
	{
        // dump($subject_list); //是有科目的
        ...
        //2019.11.12
        if($one_subject_list){
            $member_list = $member_list->where('ms.subject_id',$one_subject_list);
        }
        if($major_score_status !== null && $major_score_status !== ""){
            $member_list = $member_list->where('major_score_status',$major_score_status);
        }
        /*end*/
        ...
        $score_list = Db::name('member_list')->alias("m")
            ...
            ->join(config('database.prefix').'school sc','m.school_id = sc.school_id') //2019.11.7
            ...
            ->field('m.member_list_nickname , m.member_list_username, m.member_list_id,m.year,mj.major_name,mj.major_name,m.major_id,m.school_id,mi.ZexamineeNumber,sc.school_name');
        if($is_page)
        {
        ...
        $major_score_statuss = $major_score_status;
