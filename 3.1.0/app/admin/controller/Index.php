	...
	public function index()
	{
        ...
        if($this->admin['group_id'] == 3)
        {
            ...
            foreach($major_list as $key => $major)
            {
                ...
                $statistics[$key]['student_count'] = Db::name('member_list')->where(['major_id' => $major['major_id'],'school_id' => $this->admin['school_id']])->where(get_year_where())->count();
                $statistics[$key]['enrolment_auditing_count'] = Db::name('member_list')->where(['major_id' => $major['major_id'],'school_id' => $this->admin['school_id'],'user_status' => '1' ])->where(get_year_where())->count();
                $statistics[$key]['enrolment_unauditing_count'] = $statistics[$key]['student_count']  - $statistics[$key]['enrolment_auditing_count'];
                $statistics[$key]['score_auditing_count'] = Db::name('member_list')->alias('m')->join(config('database.prefix').'major_score ms','m.member_list_id = ms.member_list_id','left')->where('ms.major_score_status',1)->where(['m.major_id' => $major['major_id'],'m.school_id' => $this->admin['school_id']])->where(get_year_where('m'))->group('ms.member_list_id')->count();
		...
		if($this->admin['group_id'] == 4)
        {
            ...
            $map['m.year'] = ['=',$ydate]; //2019.12.20
			$statistics['student_count'] =  Db::name('member_list')->alias('m')->where($map)->count();
			$statistics['recruit_major_score_auditing_count'] = Db::name('member_list')->alias('m')->join(config('database.prefix').'recruit_major_score rms','m.member_list_id = rms.member_list_id','left')->where('rms.recruit_major_score_status',1)->where($map)->where(get_year_where('m'))->count();
            $statistics['recruit_major_score_unauditing_count'] = $statistics['student_count'] - $statistics['recruit_major_score_auditing_count'];
            $statistics['score_auditing_count'] = Db::name('member_list')->alias('m')->join(config('database.prefix').'major_score ms','m.member_list_id = ms.member_list_id','right')->where('ms.major_score_status',1)->where(get_year_where('m'))->where($map)->group('ms.member_list_id')->count();
		...
		if($this->admin['group_id'] == 5 || $this->admin['group_id'] == 1)
        {
            ...
            $school_count = Db::name('enrollment')->where(get_year_where())->group('school_id')->count();
            $major_count = Db::name('enrollment')->where(get_year_where())->group('major_ids')->count();
            $recruit_major_count = Db::name('enrollment')->where(get_year_where())->group('recruit_major_id')->count();
            
			$enroll_count = Db::name('enrollment')->where(get_year_where())->sum('enrollment_number'); //zsjh
			$student_count = Db::name('member_list')->where('year',$ydate)->count();
