...
class UniversityScore extends Base
{
    ...
    public function score_list(Request $request)
    {
        ...
        */
        ...
        // halt($nowyear);
        //获取科目
        $subject_list = SubjectModel::get_subject_list1($major_id, $school_id, $nowyear, $subject_ids);
        // dump($subject_list);
        ...
        // dump($major_score_status);
// major_subject_score_arr 问题 2019.12.28
        $data = $this->scoreModel->getMajorScoreList($map, $where, $search_key, $subject_list,1,$flag,$major_score_status);
        // dump($data);
        $score_list = $data['score_list'];
        // dump($score_list);
        $major = MajorModel::get_major_detail1($major_id, $school_id,$nowyear);
    ...
    public function score_list_export()
    {
        ...
        // halt($subject_list);
        $data = $this->scoreModel->getMajorScoreList($map,'','',$subject_list,0,$subject_ids);
...
// halt($data);
    ...
    public function score_list_export_excel()
    {
        $major_id = input('major_id','');
...
// halt($data);
        ...
        foreach ($data[0]['major_subject_score_arr'] as $k => $v) {
            $field[] = $data[0]['major_subject_score_arr'][$k]['subject_name'];
        }
       // halt($field);
        $map=["status"=>1];
        $map2=["status"=>-1];
    
        $excel->setExcelName($title)
        ->createSheet("sheet1",$data,$field,$map)
        ->downloadExcel();

        // $header =  $this->exprot_model->header_name = sprintf(config('pdf_common.header_name'),'   '.$this->recruit_major_name.'   ');
        // halt($data);
        // $this->exprot_model->university_score_list_export_pdf($field_titles,$fields,$data,$table,$title);
        // return false;
    ...
    public function major_score_import()
    {
        $school_list = $this->schoolModel->get_school_list_rmi($this->admin['recruit_major_id']);
        $school_id = input('school_id', $school_list[0]['school_id']);
        // 2020.7.3 多传入参数 $this->admin['recruit_major_id']
        $major_list = MajorModel::get_major_list($school_id,$this->admin['recruit_major_id']);
        $major_id = input('major_id', $major_list['0']['major_id']);
        $this->assign('school_list',$school_list);
        $this->assign('major_list',$major_list);
    ...
    public function major_score_export_forimport()
    {
        ...
        // 2020.7.2 采用 get_subject_list1 
        $subject_list = SubjectModel::get_subject_list1($major_id, $school_id, get_year(), $subject_ids);
